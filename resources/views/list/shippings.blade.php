                    ...
                    <div class="col-12">
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                <h3 class="card-title">Data Shippings</h3>
                                <div class="float-right">
                                    <div class="section-header-button">
                                        <a href="{{ route('shippings.create') }}" class="btn btn-primary"><i
                                    ...
                                    <tbody>
                                        @foreach ($shippings as $index => $shipping)
                                            <tr>
                                                ...
                                                <td>{{ $shipping->consigne }}</td>
                                                <td>{{ $shipping->created_at->format('d F Y') }}</td>
                                                <td align="center">
                                                    <a href="{{ route('shippings.detail', $shipping->id) }}"
                                                        class="btn btn-success btn-sm"><i class="fas fa-search"></i>
                                                        Detail</a>

                                                    <a href="{{ route('shippings.download', $shipping->id) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.switch').on('switchChange.bootstrapSwitch', function(event, state) {
                $.ajax({
                    url: '{{ route('shippings.index') }}/' + this.dataset.id + '/activate',
                    headers: {
